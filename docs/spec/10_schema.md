...
- `next_check_at` / `check_interval` are used for adaptive refresh interval scheduling. The maximum value is chosen from three signals: HTTP `Cache-Control` / `Expires`, RSS `<ttl>`, and article update frequency (half the **median** gap between consecutive articles — see [30\_ingestion.md](./30_ingestion.md#article-ingestion-pipeline) for why the median rather than the mean), then clamped to the range of 15 minutes to 4 hours. On notModified, the previous interval stored in `check_interval` is reused. The datetime format is compatible with `strftime('%Y-%m-%dT%H:%M:%SZ')` (no milliseconds)
